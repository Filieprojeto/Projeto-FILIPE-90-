/**
 * Wild Atlantic Madeira 4x4 — i18n v2
 * PT: usa sempre os textos do admin (API /api/site)
 * EN: traduz os textos do admin via /api/translate (cache localStorage)
 */

// ─────────────────────────────────────────────
// TRADUÇÕES ESTÁTICAS (fallback / UI elementos)
// Apenas para chaves que NÃO vêm do admin
// ─────────────────────────────────────────────
const STATIC_TRANSLATIONS = {
  pt: {
    ...
    sobre_tag: 'Sobre Nós', passeios_tag: 'Experiências', passeios_titulo: 'Os Nossos Passeios',
  ...
  en: {
    ...
    sobre_tag: 'About Us', passeios_tag: 'Experiences', passeios_titulo: 'Our Tours',
...
// Textos que vieram do admin (PT base)
let adminContentPT = {};

// Traduções EN dos textos do admin (cache)
// Estrutura: { cacheKey: { translatedTexts: {}, timestamp: number } }
const CACHE_TTL = 24 * 60 * 60 * 1000; // 24 horas

// Chaves do admin que queremos traduzir quando se muda para EN
const ADMIN_TRANSLATABLE_KEYS = [
  ...
  'stat1_l', 'stat2_l', 'stat3_l',
  'horario',
];

// ─────────────────────────────────────────────
// FUNÇÃO t() — resolve tradução
// ─────────────────────────────────────────────

// Textos EN do admin traduzidos (preenchido após tradução)
let adminContentEN = {};

function t(key) {
  if (currentLang === 'en') {
    // 1. Tradução EN do admin (se existir)
    if (adminContentEN[key] !== undefined) return adminContentEN[key];
    // 2. Tradução estática EN
    ...
    // 4. Fallback PT estático
    return STATIC_TRANSLATIONS.pt[key] || key;
  }
  // PT: usa admin se existir, senão estático
  if (adminContentPT[key] !== undefined) return adminContentPT[key];
  return STATIC_TRANSLATIONS.pt[key] || key;
}
...
function getCacheKey() {
  // A chave de cache é um hash dos textos PT do admin
  // Assim se o admin mudar um texto, o cache invalida
  const str = ADMIN_TRANSLATABLE_KEYS.map(k => adminContentPT[k] || '').join('|');
  return 'wam_trans_en_' + btoa(unescape(encodeURIComponent(str))).slice(0, 32);
}
...
function loadTranslationCache() {
  try {
    ...
    if (Date.now() - data.timestamp > CACHE_TTL) { localStorage.removeItem(key); return null; }
...
function saveTranslationCache(translations) {
  try {
    ...
    // Limpa caches antigos (mantém só os últimos 3)
    const keys = Object.keys(localStorage).filter(k => k.startsWith('wam_trans_en_'));
    if (keys.length > 3) {
...
async function translateAdminTexts() {
  // Só traduz as chaves que têm conteúdo do admin E são traduzíveis
  const toTranslate = {};
  ADMIN_TRANSLATABLE_KEYS.forEach(key => {
    if (adminContentPT[key]) toTranslate[key] = adminContentPT[key];
  });
  ...
  if (cached) {
    console.log('[i18n] Usando cache de tradução EN');
    return cached;
  }

  console.log('[i18n] A traduzir textos do admin para EN...');
  try {
    ...
    console.warn('[i18n] Tradução falhou, usando textos PT:', err.message);
  }
  return {};
}

// ─────────────────────────────────────────────
// MAPEAMENTO: conteúdo do admin → chaves i18n
// ─────────────────────────────────────────────
function mapAdminContent(content) {
  // O /api/site devolve content com as chaves do servidor
  // Aqui mapeamos para as chaves usadas no i18n
  const mapped = {};
  ...
  directMap.forEach(key => {
    if (content[key] !== undefined && content[key] !== '') {
      mapped[key] = content[key];
    }
  });
...
function applyTranslations() {
  ...
  // Atualiza também os números das stats se vieram do admin
  if (adminContentPT['stat1_n']) set('stat1n', adminContentPT['stat1_n']);
  if (adminContentPT['stat2_n']) set('stat2n', adminContentPT['stat2_n']);
  if (adminContentPT['stat3_n']) set('stat3n', adminContentPT['stat3_n']);
  ...
  if (adminContentPT['sobre_anos']) set('sobreAnos', adminContentPT['sobre_anos']);
  ...
  // Cards dos passeios — apenas se não foram carregados do servidor
  // (se vieram do servidor, o main.js trata disso; aqui só traduzimos o que está no DOM)
  ...
  cards.forEach((card, i) => {
    const pd = passData[i]; if (!pd) return;
    ...
    if (priceSpan) {
      if (i === 5) {
        ...
        if (strong) {
          ...
          priceSpan.innerHTML = lang === 'en'
            ? `${t('preco_from')} <strong>${amount}</strong>/person`
            : `${t('preco_from')} <strong>${amount}</strong>/pessoa`;
        }
      }
    }
  ...
  const formLabels = { 'nome':t('form_nome'), 'email':t('form_email'), 'telefone':t('form_telefone'), 'data':t('form_data'), 'pessoas':t('form_pessoas'), 'passeio':t('form_passeio'), 'mensagem':t('form_mensagem') };
  Object.entries(formLabels).forEach(([id, text]) => {
    const lbl = document.querySelector(`label[for="${id}"]`); if (lbl) lbl.textContent = text;
  });
  ...
  if (Array.isArray(aItems)) asideItems.forEach((li, i) => { if (aItems[i]) li.textContent = aItems[i]; });
  ...
  if (waBtn) {
    ...
    const waMsgKey = lang === 'en' ? t('whatsapp_msg') : 'Olá! Gostaria de saber mais sobre os vossos passeios.';
    const waTel = adminContentPT['whatsapp'] || '351912345678';
    waBtn.href = `https://wa.me/${waTel}?text=${encodeURIComponent(t('whatsapp_msg'))}`;
  }
  ...
  // Horário dinâmico do admin
  const horarioEl = document.getElementById('contactoHorario');
  if (horarioEl) {
    const horVal = lang === 'en' && adminContentEN['horario']
      ? adminContentEN['horario']
      : (adminContentPT['horario'] || STATIC_TRANSLATIONS.pt['contacto_horario_val']);
    horarioEl.textContent = horVal;
  }

  // Telefone/email dinâmicos
  if (adminContentPT['telefone']) {
    const telEl = document.getElementById('contactoTelefone');
    if (telEl) telEl.textContent = adminContentPT['telefone'];
  }
  if (adminContentPT['email']) {
    const emailEl = document.getElementById('contactoEmail');
    if (emailEl) emailEl.textContent = adminContentPT['email'];
    const linkEl = document.getElementById('linkEmail');
    if (linkEl) linkEl.href = 'mailto:' + adminContentPT['email'];
  }
  ...
  ['footer_passeios','footer_empresa','footer_legal'].forEach((k, i) => { if (ftCols[i]) ftCols[i].textContent = t(k); });
  const empLinks = document.querySelectorAll('.footer-grid > div:nth-child(3) li a');
  ['footer_sobre','footer_galeria','footer_videos','footer_testemunhos','footer_contacto'].forEach((k, i) => { if (empLinks[i]) empLinks[i].textContent = t(k); });
  const legLinks = document.querySelectorAll('.footer-grid > div:nth-child(4) li a');
  ['footer_privacidade','footer_termos','footer_cancelamento','footer_reclamacoes'].forEach((k, i) => { if (legLinks[i]) legLinks[i].textContent = t(k); });
  ...
  // WhatsApp float — atualiza mensagem
  const waTel2 = adminContentPT['whatsapp'] || '351912345678';
  document.querySelectorAll('a[href*="wa.me"]').forEach(a => {
    a.href = `https://wa.me/${waTel2}?text=${encodeURIComponent(t('whatsapp_msg'))}`;
  });
...
function createLangToggle() {
  ...
  style.textContent = `
    ...
    .lang-loading { opacity:.5;pointer-events:none; }
...
async function setLang(lang, save = true) {
  if (lang === currentLang && Object.keys(adminContentPT).length > 0) {
    // Já está no idioma certo, apenas re-aplica
  ...
  if (lang === 'en' && Object.keys(adminContentPT).length > 0) {
    // Mostra loading no toggle
    const enBtn = document.getElementById('langBtnEN');
    if (enBtn) enBtn.classList.add('lang-loading');

    // Tenta traduzir (usa cache se disponível)
    const translated = await translateAdminTexts();
    adminContentEN = translated || {};

    if (enBtn) enBtn.classList.remove('lang-loading');
  }
...
// ─────────────────────────────────────────────
// BANNER "DETECTED ENGLISH"
// ─────────────────────────────────────────────
function showEnglishBanner() {
  if (document.getElementById('langBanner')) return;
  const banner = document.createElement('div');
  banner.id = 'langBanner';
  banner.innerHTML = `🇬🇧 We detected you may prefer English. <button onclick="setLang('en')">Switch to English</button> <button id="langBannerDismiss" onclick="this.closest('#langBanner').remove()">✕</button>`;
  document.body.appendChild(banner);
  setTimeout(() => banner.parentNode && banner.remove(), 10000);
}
...
async function detectLangByIP() {
  ...
  if (saved) { await setLang(saved, false); return; }
  try {
...
// ─────────────────────────────────────────────
// INIT — carrega conteúdo do admin PRIMEIRO
// ─────────────────────────────────────────────
async function initI18n() {
  createLangToggle();

  // Carrega conteúdo do servidor (PT base)
  try {
    const resp = await fetch('/api/site', { signal: AbortSignal.timeout(5000) });
    if (resp.ok) {
      const data = await resp.json();
      if (data.content) {
    ...
    console.warn('[i18n] Não foi possível carregar conteúdo do admin:', err.message);
  }

  // Agora deteta idioma (e traduz se necessário)
